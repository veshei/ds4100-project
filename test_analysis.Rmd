...
data_get <- function(ticker) {
  av_get(symbol = ticker, av_fun = "TIME_SERIES_DAILY_ADJUSTED", interval = "60min", outputsize = "full")
}
```

```{r}
# Alpha Vantage API key: KEI93A3FCAX5K5R9
sp500_idx <- data_get("^GSPC")
# Alpha Vantage API key: KEI93A3FCAX5K5R9
fb <- data_get("FB")
```
...
```{r}
#sp500_idx = cbind(sp500_idx,ewma=EWMA(sp500_idx$adjusted_close, 1, startup = 0))
sp500_idx$ewma = EWMA(sp500_idx$adjusted_close, .1, startup = 0)
sp500_idx
```

```{r}
yr1_range = nrow(sp500_idx) - 365
full_range = nrow(sp500_idx)
sp500_short = sp500_idx[yr1_range:full_range,]
ggplot(sp500_short,aes(x=timestamp)) + geom_line(aes(y=ewma),color="Red") + geom_line(aes(y=adjusted_close))
```
