...
get_percent_values <- function(ticker_df, type) {
  if (type == "price") {
    curr_val <- (dplyr::lead(ticker_df$Adjusted_Close, 1) - ticker_df$Adjusted_Close) / ticker_df$Adjusted_Close * 100
  } else if (type == "volume") {
    curr_val <- (dplyr::lead(ticker_df$Volume, 1) - ticker_df$Volume) / ticker_df$Volume * 100
  }
...
dbWriteTable(conn=db, name="MMM", value=MMM_df, row.names = FALSE, append = TRUE)
```
...
for (curr_tick in sp500_tickers) {
  df_name <- paste(curr_tick, "_df", sep = "")
  dbWriteTable(conn = db, name = as.character(curr_tick), value = get(df_name),
             row.names = FALSE, overwrite = TRUE)
  head(dbReadTable(conn = db, as.character(curr_tick)))
}
dbReadTable(conn = db, as.character(curr_tick))
```
