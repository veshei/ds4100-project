...
```{r}
mmm_ewma = EWMA(MMM_df$adjClosing, lambda = .1) #exponentially weighted moving average (EMA)
mmm_ewma
```

```{r}
mmm_sma = SMA(MMM_df$adjClosing) #simple moving average
mmm_sma = c(c(0,0,0,0), mmm_sma)
mmm_sma
```

```{r}
mmm_mom = momTA(MMM_df$adjClosing) #momentum 
mmm_mom
```

```{r}
mmm_macd = macdTA(MMM_df$adjClosing) #moving average conversion to divergents
mmm_macd
```

```{r}
mmm_stoch = stochasticTA(MMM_df$adjClosing, MMM_df$high, MMM_df$low)
mmm_stochk = mmm_stoch[,1]
mmm_stochk
mmm_stochd = mmm_stoch[,2]
mmm_stochd
```

```{r}
mmm_rsi = rsiTA(MMM_df$adjClosing) #relative strength index
mmm_rsi
```

```{r}
mmm_vor = vorTA(MMM_df$high, MMM_df$low) #volatility ratio
mmm_vor
```

```{r}
MMM_df = data_frame(
  Adjusted_Close = MMM_df$adjClosing,
  Volume = MMM_df$volume,
  SMA = mmm_sma,
  EMA = mmm_ewma,
  MOM = mmm_mom,
  MACD = mmm_macd,
...
```{r}
dbListTables(ds4100_project_db)
```
