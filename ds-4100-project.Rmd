...
get_stock_data <- function(ticker) {
  ...
  dates <- dates[which(!dates %in% split_dates)]
...
for(ticker in sp500_tickers) {
  ...
  print("ticker error")
  skipped_tickers <- skipped_tickers + c(ticker)
})
...
```{r}
EWMA(MMM_df$adjClosing, lambda = .1) #exponentially weighted moving average (EMA)
```
