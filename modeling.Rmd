...
for (ticker in dbListTables(db)) {
  ...
  master_dataset = rbind(master_dataset, curr_data[,7:length(curr_data)])
}
...
correlations <-  cor(master_dataset %>% select(PERCENT_P))
...
```{r}
# Linear Model
pred_lm_short <- step(lm(PERCENT_CHANGE_20 ~ PERCENT_PRICE + NORM_SMA + NORM_EWMA + MOM + MACD + STOCH.K + STOCH.D + RSI + VOR, train_df), direction = "both")
...
```{r}
# Logistic Model
pred_glm_short <- step(glm(FUTURE_CLASS_20 ~ PERCENT_PRICE + NORM_SMA + NORM_EWMA + MOM + MACD + STOCH.K + STOCH.D + RSI + VOR, train_df, family=binomial), direction = "both")
